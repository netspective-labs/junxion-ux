...
export class NaturalGrid extends HTMLElement {
  ...
  #render() {
    ...
    if (this.#viewportElement && this.#viewportScrollHandler) {
      this.#viewportElement.removeEventListener("scroll", this.#viewportScrollHandler);
    ...
    const viewportHeightPx = Math.max(160, Number(virtualizationUi.viewportHeightPx ?? 360));
    const bufferRows = Math.max(0, Number(virtualizationUi.bufferRows ?? 5));
    const columnWidthHint = Math.max(48, Number(virtualizationUi.columnWidthPx ?? 160));
    const columnBuffer = Math.max(0, Number(virtualizationUi.columnBuffer ?? 2));
    const viewportWidthPx = Math.max(200, Number(virtualizationUi.viewportWidthPx ?? 760));
    ...
    const visibleRowCount = virtualizationEnabled
      ? Math.max(
        1,
        Math.min(totalFlatRows, Math.ceil(viewportHeightPx / rowHeight) + bufferRows * 2),
      )
      : totalFlatRows;
    ...
    const columnWidths = cols.map((c) => Math.max(1, Number(c.widthPx ?? columnWidthHint)));
    ...
    const leftPadding = virtualizationEnabled ? (columnOffsets[colStart] ?? 0) : 0;
    const rightPadding = virtualizationEnabled
      ? Math.max(0, totalColumnWidth - (columnOffsets[colEnd] ?? totalColumnWidth))
      : 0;
    ...
    if (virtualizationEnabled) {
      ...
      const safeColIndex = Math.min(this.#virtualizationState.colStart, columnOffsets.length - 1);
