...
export function createCxUserAgent(cfg = {}) {
    ...
    const diag = (kind, data = {}) => {
        ...
        try {
            // eslint-disable-next-line no-console
            console.log(CX_DIAG_PREFIX, JSON.stringify({ kind, ts: Date.now(), data }));
        } catch {
            // ignore
        }
    };
    ...
    const safeExec = (jsText) => {
        ...
        try {
            ...
            diag("exec:end", { ok: false, error: String(err && (err.stack || err.message || err)) });
    ...
    const connectSse = () => {
        ...
        try {
            ...
            es = null;
            diag("sse:init_error", { error: String(err && (err.stack || err.message || err)) });
