...
const applyPatchOps = (patch) => {
  if (!patch || !Array.isArray(patch.ops)) return;
  for (const op of patch.ops) {
    if (!op || typeof op !== "object" || typeof op.op !== "string") continue;
    switch (op.op) {
      case "setText": {
        const el = querySelectorSafe(op.selector);
        if (el) el.textContent = String(op.text ?? "");
        break;
      }
      case "setAttribute": {
        const el = querySelectorSafe(op.selector);
      ...
      case "setDataset": {
        ...
        if (!el) break;
        if (op.value == null) {
          delete el.dataset[op.key];
        } else {
          el.dataset[op.key] = String(op.value);
        }
        break;
      }
      case "toggleClass": {
        ...
        if (op.active) el.classList.add(op.className);
...
export class CxAide {
  ...
  constructor(host) {
    ...
    this.#patchEventName = kDefault.patchEventName;
  ...
  sseConnect(opts = {}) {
    ...
    this.#patchEventName = String(
      opts.patchEventName ||
        opts.ssePatchEventName ||
        this.#patchEventName ||
        kDefault.patchEventName,
    );
    ...
    this.#patchListener = (ev) => {
      try {
        applyPatchOps(jsonFromEvent(ev));
      } catch {
        // ignore
      }
    };
    try {
      this.#es.addEventListener(this.#patchEventName, this.#patchListener);
    } catch {
      // ignore
    }
  ...
  if (autoConnect) {
    try {
      aide.sseConnect({
        ...
        patchEventName: opts.ssePatchEventName || opts.patchEventName,
